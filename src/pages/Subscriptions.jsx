...
// Normalise a subscription's amount to a monthly KES equivalent
function monthlyKes(sub) {
  const base = sub.amount_kes ?? sub.amount * (sub.rate_kes ?? 130);
  const multipliers = { monthly: 1, yearly: 1 / 12, weekly: 52 / 12, one_time: 0 };
  return base * (multipliers[sub.billing_cycle] ?? 1);
}
...
export default function Subscriptions() {
  ...
  async function handleSave(payload) {
    ...
    try {
      ...
      setFormError(
        err.body?.description?.errors?.join(", ") ?? err.message
      );
    } finally {
      setSaving(false);
    }
